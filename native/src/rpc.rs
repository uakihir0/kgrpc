...
pub unsafe extern "C" fn rpc_implementation(
    ...
    let on_done_error: fn(RawPtr, &str, RpcOnDone) =
        |user_data: RawPtr, err_str: &str, on_done: RpcOnDone| {
    ...
    let path = unsafe {
        match CStr::from_ptr(path)
            ...
            .and_then(|s| {
                PathAndQuery::from_maybe_shared(s).map_err(|e| e.to_string())
            }) {
...
unsafe fn create_rpc_future(
    ...
    TOKIO_RT.spawn(async move {
        ...
        let on_done_cancelled = |reason: String| on_done_with_status(Status::cancelled(reason));
        ...
        // Start request
        let start_streaming_result: Result<RpcResult<Response<Streaming<RawPtr>>>, String> =
            tokio::select! {
                r = async {
                    match build_and_wait_for_grpc(channel).await {
                        Ok(mut grpc) => {
                            match grpc.streaming(request_stream, path, codec).await {
                ...
                _ = async { shutdown_rx.changed().await } => {
                    trace!("rpc_implementation() - shutdown_rx.changed() returned");
        ...
        let mut body = match start_streaming_result {
            Ok(rpc_result) => match rpc_result {
                ...
                RpcResult::Status(status) => {
                    trace!(
                        "rpc_implementation() - grpc.streaming returned STATUS with code {}",
                        i32::from(status.code())
                    );
        ...
        let call_result: RpcResult<Option<MetadataMap>> = tokio::select! {
            ...
            _ = async { shutdown_rx.changed().await } => {
                trace!("rpc_implementation() - shutdown_rx.changed() returned");
                drop(body);
                Cancelled("rpc cancelled while reading body".to_string())
            }
        };
...
async fn build_and_wait_for_grpc(channel: Channel) -> Result<Grpc<Channel>, String> {
